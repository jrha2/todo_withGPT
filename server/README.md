...
`apps/desktop`에서 `npm run dist:win`을 실행하면 설치파일, blockmap, 업데이트 채널
메타데이터(`beta.yml` 또는 `latest.yml`)가
`server/updates`에 자동 복사됩니다. 앱은 실행할 때와 서버에 다시 연결할 때
`/updates` 주소를 확인합니다. 이후 새 배포파일이 등록되면 서버가 연결된 앱으로
Push 신호를 보내 즉시 업데이트 확인을 시작하고, 사용자의 승인을 받아 새 버전을
다운로드한 뒤 재시작하면서 설치합니다.

0.9.3 Beta는 자동 업데이트 기능이 처음 포함된 버전입니다. 0.9.3 사용자는 기존의
주기 확인으로 0.9.4를 받을 수 있고, 0.9.4부터는 서버 Push 방식으로 다음 버전을
확인합니다.
